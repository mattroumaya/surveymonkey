...
responses_raw <- readRDS("tests/testthat/test_responses_raw.rds")
...
parsed_correctly <- readRDS("tests/testthat/parsed_responses.rds")

test_that("responses parsed correctly", {
  expect_equal(
    parsed_correctly,
    parse_respondent_list(responses_raw)
  )
})
